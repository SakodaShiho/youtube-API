          ...
          {props.videos.map((video,index) => (
            <div key={video.id.videoId}>
              <Card className={`border-none ${index % 2 == 0 ? '' : 'gray'}`}>
                ...
                <a href={`https://youtube.com/watch?v=${video.id.videoId}`} target="_blank"><div className="title">{video.snippet.title}</div></a>
                <div className="accordion-card">
                <Accordion>
                  
                    <Accordion.Toggle as={Card.Header} eventKey="0" className="accordion-btn">
                      ▼
                    </Accordion.Toggle>
